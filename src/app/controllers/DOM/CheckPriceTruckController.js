const DomTruck = require("../../models/DOM/CheckPriceTruck");

class CheckPriceTruckController {
  //[GET] /truck/getAll
  async getAll(req, res) {
    try {
      const checktruck = await DomTruck.find({});
      res.status(200).json({
        success: true,
  ...
  async deleteTruck(req, res) {
    try {
      const id = await DomTruck.findByIdAndDelete(req.params._id);
      res.status(200).json({
        success: true,
        message: "delete Truck successfully",
        id,
      });
      console.log("delete Truck successfully");
    } catch (error) {
      console.log(error);
...
module.exports = new CheckPriceTruckController();
