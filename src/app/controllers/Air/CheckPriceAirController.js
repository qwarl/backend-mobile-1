const Air = require("../../models/Air/CheckPriceAir");

class CheckPriceAirController {
  //[GET] /air/getAll
  async getAll(req, res) {
    try {
      const CheckPriceAir = await Air.find({});
      res.status(200).json({
        success: true,
  ...
  async deleteAir(req, res) {
    try {
      const id = await Air.findByIdAndDelete(req.params._id);
      res.status(200).json({
        success: true,
...
module.exports = new CheckPriceAirController();
