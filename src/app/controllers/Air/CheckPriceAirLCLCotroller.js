const LCL = require("../../models/Air/CheckPriceAirLCL");

class CheckPriceAirLCLCotroller {
  //[GET] /air/getAll
  async getAll(req, res) {
    try {
      const checkPriceAirLCL = await LCL.find({});
      res.status(200).json({
        success: true,
        message: "Get all air successfully",
        checkPriceAirLCL,
      });
  ...
  async update(req, res) {
    try {
      const updateLCL = await LCL.findByIdAndUpdate(req.params._id, req.body, {
        new: true,
      });
      res
        .status(200)
        .json({ success: true, message: "Update air successfully", updateLCL });
  ...
  async deleteLCL(req, res) {
    try {
      const id = await LCL.findByIdAndDelete(req.params._id);
      res.status(200).json({
        success: true,
        message: "delete LCL successfully",
        id,
      });
      console.log("delete LCL successfully");
    } catch (error) {
      console.log(error);
  ...
  async search(req, res) {
    try {
      ...
      if (aol && month && continent) {
        const lcls = await LCL.find({
          $and: [{ pol: pol }, { month: month }, { continent: continent }],
        });
        return res
          .status(200)
          .json({ success: true, message: "Search air successfully", lcls });
...
module.exports = new CheckPriceAirLCLCotroller();
