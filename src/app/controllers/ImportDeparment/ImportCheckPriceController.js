const Import = require("../../models/ImportDeparment/ImportCheckPrice");

class ImportCheckPriceController {
  //[GET] /Imports/getAll
  async getAll(req, res) {
    try {
      const dataImportCheckPrice = await Import.find({});
      res.status(200).json({
        success: true,
        message: "Get all import successfully",
        dataImportCheckPrice,
      });
      // console.log('Get all Import successfully');
    } catch (error) {
      console.log(error);
      res.status(400).json({ success: false, message: "Get failed" });
    }
  }

  //[POST] /Imports/create
  async create(req, res) {
    try {
      const newImport = new Import(req.body);
      let newCode = await Import.findOne().sort({ _id: -1 }).limit(1);
      // console.log(newCode.code);
      let counter;
      if (!newCode) {
        counter = 0;
      } else {
        counter = newCode.code.toString().slice(-3);
      }
      console.log((+counter + 1).toString().length);
      let getDate = new Date();
      let month = getDate.getMonth() + 1;
      month = month.toString().padStart(2, "0");
      let year = getDate.getFullYear().toString().slice(-2);
      let subCode = "BGIMPORT" + year + month;
      var code;
      let check = month;

      if ((check = month)) {
        counter++;
        // if(counter)
        let ait = counter.toString().padStart(3, "0"); // ati =auto increment code
        code = subCode + ait; // if ati>1000, it will become ati=001 again, solve this by change padStart(3,'0') to padStart(4,'0')
      } else {
        check = month;
        counter = 0;
        counter++;
        let ati = number.toString().padStart(3, "0");
        code = subCode + ati;
      }
      console.log(code);
      newImport.code = code;
      // console.log(newImport);
      await newImport.save();
      res.status(200).json({
        success: true,
        message: "Create new Import successfully",
        newImport,
      });
    } catch (error) {
      console.log(error);
      res.status(400).json({ success: false, message: "Create failed" });
    }
  }

  //[POST] /Imports/update
  async update(req, res) {
    try {
      const updateImport = await Import.findByIdAndUpdate(
        req.params._id,
        req.body,
        { new: true }
      );
      res.status(200).json({
        success: true,
        message: "Update Import successfully",
        updateImport,
      });
      console.log("Update Import successfully");
    } catch (error) {
      console.log(error);
      res.status(400).json({ success: false, message: "Update failed" });
    }
  }

  async deleteImport(req, res) {
    try {
      const id = await Import.findByIdAndDelete(req.params._id);
      res.status(200).json({
        success: true,
        message: "delete Import successfully",
        id,
      });
      console.log("delete Import successfully");
    } catch (error) {
      console.log(error);
      res.status(400).json({ success: false, message: "delete failed" });
    }
  }

  //[GET] /Imports/search
  async search(req, res) {
    try {
      const { pol, pod, type, month, continent } = req.query;
      // console.log(pol, pod, type, month, continent);

      //code sua

      if (month && continent && type) {
        const Imports = await Import.find({
          $and: [{ month: month }, { continent: continent }, { type: type }],
        });
        return res.status(200).json({
          success: true,
          message: "Search Import successfully",
          Imports,
        });
        console.log("Search Import successfully");
      }
      if (month && continent && type) {
        const Imports = await Import.find({
          $and: [{ month: month }, { continent: continent }, { type: type }],
        });
        return res.status(200).json({
          success: true,
          message: "Search Import successfully",
          Imports,
        });
        console.log("Search Import successfully");
      }
      if (month && continent) {
        const Imports = await Import.find({
          $and: [{ month: month }, { continent: continent }],
        });
        return res.status(200).json({
          success: true,
          message: "Search Import successfully",
          Imports,
        });
        console.log("Search Import successfully");
      }
      if (month && type) {
        const Imports = await Import.find({
          $and: [{ month: month }, { type: type }],
        });
        return res.status(200).json({
          success: true,
          message: "Search Import successfully",
          Imports,
        });
        console.log("Search Import successfully");
      }
      if (continent && type) {
        const Imports = await Import.find({
          $and: [{ continent: continent }, { type: type }],
        });
        return res.status(200).json({
          success: true,
          message: "Search Import successfully",
          Imports,
        });
        console.log("Search Import successfully");
      }

      if (pol && pod && type) {
        const Imports = await Import.find({
          $and: [{ pol: pol }, { pod: pod }, { type: type }],
        });
        return res.status(200).json({
          success: true,
          message: "Search Import successfully",
          Imports,
        });
        console.log("Search Import successfully");
      }
      if (pol && pod) {
        const Imports = await Import.find({
          $and: [{ pol: pol }, { pod: pod }],
        });
        return res.status(200).json({
          success: true,
          message: "Search Import successfully",
          Imports,
        });
        console.log("Search Import successfully");
      }
      if (pol && type) {
        const Imports = await Import.find({
          $and: [{ pol: pol }, { type: type }],
        });
        return res.status(200).json({
          success: true,
          message: "Search Import successfully",
          Imports,
        });
        console.log("Search Import successfully");
      }
      if (pod && type) {
        const Imports = await Import.find({
          $and: [{ pod: pod }, { type: type }],
        });
        return res.status(200).json({
          success: true,
          message: "Search Import successfully",
          Imports,
        });
        console.log("Search Import successfully");
      } else {
        const Imports = await Import.find({
          $or: [
            { pol: pol },
            { pod: pod },
            { type: type },
            { month: month },
            { continent: continent },
          ],
        });
        return res.status(200).json({
          success: true,
          message: "Search Import successfully",
          Imports,
        });
        console.log("Search Import successfully");
      }
    } catch (error) {
      console.log(error);
      return res.status(400).json({ success: false, message: "Search failed" });
    }
  }
}

module.exports = new ImportCheckPriceController();
